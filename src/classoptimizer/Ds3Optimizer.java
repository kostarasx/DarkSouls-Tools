...
/*
 * Class variables allClasses An array with the 10 dark souls 3 classes newLevel
 * An array with 10 integers. Each element is the soul level of a game class
 * classPointer An array with 10 integers. Each elements at first is the index
 * of the array in allClasses array After the sort based of the the level, each
 * elements is also sorted
 *
 * Stats[] 0.Knight 0.Level 1.Mercenary 1.Vigor 2.Warrior 2.Attunement 3.Herald
 * 3.Endurance 4.Thief 4.Vitality 5.Assassin 5.Strength 6.Sorcerer 6.Dexterity
 * 7.Pyromancer 7.Intelligence 8.Cleric 8.Faith 9.Deprived 9.Luck
 */
public class Ds3Optimizer {
	...
	/**
	 * Store the base stats for each of game class for later usage. For example user
	 * maybe wants to entered the stats for the beginning so the stats must be
	 * reseted using the baseStats array
	 */
	...
	/**
	 * Reset the the stats array with the baseStats array. Also calling the
	 * resetNewLevelArray
	 */
	 ...
	 * <p>
	 * If new value is bigger than the previous update the stat and increase the
	 * level based of the difference.\ between the new and old value.</br>
	 * If the value is smaller but not 0, update the stat and decrease the level
	 * based of the difference between the old value and the new value. Also check
	 * if the value is not smaller than the starting value of each gaming
	 * class.</br>
	 * Last if the value is 0 means that the user dont care about the value when he
	 * makes his build. So the stat for each gaming class set to the staring value
	 * and update the level with the differenced betwenn old value and the base
	 * stat.
	 * <p>
	 * 0.Soul Level</br>
	 * 1.Vigor</br>
	...
	/**
	 * Set the newLevel array with the starting soul level and set the classPointer
	 * array.
	 */
	...
	/**
	 * Every time a stat changes the newLevel array musy be update with the new
	 * level
	 */
	...
	/**
	 * A bubble sort for the new level array. The minimum element is first. Also
	 * classpoint changes also following the changes of the newLevel array.
	 */
